...
const qsa = <T extends HTMLElement>(selector: string) => Array.from(document.querySelectorAll(selector)) as T[];
...
const langSwitchBtn = qs<HTMLButtonElement>('#lang-switch');
const authLinkTop = qs<HTMLAnchorElement>('#auth-link-top');
const authUsername = qs<HTMLSpanElement>('#auth-username');
const adminPanelBtn = qs<HTMLButtonElement>('#admin-panel-btn');
...
function showAuthMode(mode: 'login' | 'register'): void {
  ...
  const authTitle = qs<HTMLHeadingElement>('.auth-header h2');
...
async function initLocale(): Promise<void> {
  const { data } = await api<{ locale: 'ru' | 'en'; translations: Translations }>('/api/locale');
  if (data) {
...
function updateUIText(): void {
  if (logo) logo.textContent = t('header.title');
  const sidebar = document.querySelector('.sidebar-footer');
  if (sidebar) sidebar.textContent = t('header.title');
  
  qsa<HTMLAnchorElement>('.sidebar-item').forEach(item => {
    const href = item.getAttribute('href');
    const key = href?.slice(1);
    if (key === 'items') item.textContent = t('sidebar.items');
    if (key === 'cart') item.textContent = t('sidebar.cart');
    if (key === 'my-lots') item.textContent = t('sidebar.myLots');
    if (key === 'inventory') item.textContent = t('sidebar.inventory');
    if (key === 'admin') item.textContent = t('sidebar.admin');
  });

  if (composeBtn) composeBtn.textContent = t('toolbar.sell');
  if (langSwitchBtn) langSwitchBtn.textContent = currentLocale === 'ru' ? 'EN' : 'RU';
  if (searchInput) searchInput.placeholder = t('toolbar.search');
  if (sortInput) {
  ...
  if (applyFiltersBtn) applyFiltersBtn.textContent = t('toolbar.apply');

  const authTitle = qs<HTMLHeadingElement>('.auth-header h2');
  if (authTitle) authTitle.textContent = t('auth.login');

  const showLoginBtn = qs<HTMLButtonElement>('#show-login');
  const showRegisterBtn = qs<HTMLButtonElement>('#show-register');
  if (showLoginBtn) showLoginBtn.textContent = t('auth.login');
  if (showRegisterBtn) showRegisterBtn.textContent = t('auth.register');
  ...
  const loginBtn = qs<HTMLButtonElement>('#login-btn');
  const regBtn = qs<HTMLButtonElement>('#reg-btn');

  if (loginNameInput) loginNameInput.placeholder = t('auth.username');
  if (loginPassInput) loginPassInput.placeholder = t('auth.password');
  if (regNameInput) regNameInput.placeholder = t('auth.username');
  if (regPassInput) regPassInput.placeholder = t('auth.password');
  if (regEmailInput) regEmailInput.placeholder = t('auth.email');
  if (regPhoneInput) regPhoneInput.placeholder = t('auth.phone');
  if (loginBtn) loginBtn.textContent = t('auth.login');
  if (regBtn) regBtn.textContent = t('auth.register');
}

async function showLocaleSelector(): Promise<void> {
  const existing = qs<HTMLDivElement>('#locale-selector');
  if (existing) existing.remove();

  const { data } = await api<{ detectedLocale: 'ru' | 'en'; country: string }>('/api/locale/detect');
  ...
  qs<HTMLButtonElement>('#locale-yes')?.addEventListener('click', () => {
    selector.remove();
  ...
  qs<HTMLButtonElement>('#locale-no')?.addEventListener('click', () => {
...
async function setLocale(locale: 'ru' | 'en'): Promise<void> {
  const { data } = await api<{ locale: 'ru' | 'en'; translations: Translations }>('/api/locale', {
    method: 'POST',
...
async function loadProducts(): Promise<void> {
  ...
  if (currentUser) {
    const rec = await api<{ recommendations: Product[] }>('/api/recommendations');
...
async function renderList(): Promise<void> {
  ...
  if (currentView === 'items') {
    ...
    qsa<HTMLButtonElement>('.btn-reviews').forEach(button => {
      button.addEventListener('click', async event => {
        event.stopPropagation();
        const id = Number(button.getAttribute('data-id'));
        if (!Number.isNaN(id)) await showDetail(id);
      });
    });
    ...
    qsa<HTMLButtonElement>('.btn-add').forEach(button => {
      button.addEventListener('click', async event => {
        ...
        if (res.ok) showModal(`<p>${t('messages.added')}</p>`);
        else showModal(`<p>${t('messages.error')}</p>`);
      });
    });
    return;
  }
  ...
  if (currentView === 'inventory') {
    const { res, data } = await api<Array<{ name: string; price: number; boughtAt: string }>>('/api/inventory');
    if (!res.ok || !data) {
      listEl.innerHTML = `<p>${t('messages.loginRequired')}</p>`;
      return;
    }
...
async function showDetail(id: number): Promise<void> {
  ...
  const { data: reviewData } = await api<{ reviews: Review[]; averageRating: number; count: number }>(`/api/reviews/${id}`);
  ...
  qs<HTMLButtonElement>('#like-btn')?.addEventListener('click', async () => {
    if (!currentUser) {
      window.location.hash = '#login';
      return;
    }
    const { res } = await api(`/api/likes/${id}`, { method: 'POST' });
    if (res.ok) showModal('<p>Liked ❤️</p>');
  });

  qs<HTMLButtonElement>('#submit-review')?.addEventListener('click', async () => {
    const rating = Number(qs<HTMLSelectElement>('#review-rating')?.value ?? 5);
    const comment = (qs<HTMLTextAreaElement>('#review-comment')?.value ?? '').trim();
    if (!comment) {
      showModal(`<p>${t('messages.error')}</p>`);
      return;
    }
...
async function buyProduct(id: number): Promise<void> {
  const { res, data } = await api<{ message?: string }>('/api/buy/' + id, { method: 'POST' });
  if (res.ok) {
    await loadProducts();
    await renderList();
    showModal(`<p>${t('messages.purchased')}</p>`);
  } else {
    showModal(`<p>${escapeHtml((data && (data as any).message) || t('messages.error'))}</p>`);
  }
}
...
async function loadAdminPanel(): Promise<void> {
  ...
  qs<HTMLButtonElement>('#add-product-btn')?.addEventListener('click', () => {
    showProductForm(null);
  });

  qsa<HTMLButtonElement>('.btn-edit').forEach(btn => {
  ...
  qsa<HTMLButtonElement>('.btn-delete').forEach(btn => {
    btn.addEventListener('click', async () => {
      const id = Number(btn.getAttribute('data-id'));
  ...
  qsa<HTMLButtonElement>('.btn-block').forEach(btn => {
    btn.addEventListener('click', async () => {
      const id = Number(btn.getAttribute('data-id'));
      const product = products.find(p => p.id === id);
      if (!product) return;
...
function showProductForm(product: Product | null): void {
  if (!detailEl) return;
  ...
  const selectRarity = qs<HTMLSelectElement>('#rarity');
  if (selectRarity && product?.rarity) {
    selectRarity.value = product.rarity;
  }

  qs<HTMLButtonElement>('#product-cancel')?.addEventListener('click', async () => {
    await loadAdminPanel();
  });

  qs<HTMLButtonElement>('#product-submit')?.addEventListener('click', async () => {
    const values = {
      ...
      count: Number(qs<HTMLInputElement>('#count')?.value ?? 1),
      description: (qs<HTMLTextAreaElement>('#desc')?.value ?? '').trim(),
      tags: (qs<HTMLInputElement>('#tags')?.value ?? '').trim()
    };
...
function attachAuthEvents(): void {
  ...
  qs<HTMLButtonElement>('#login-btn')?.addEventListener('click', async () => {
    ...
    if (!name || !password) {
      if (authMsg) authMsg.textContent = t('messages.error');
      return;
    }
    const { res, data } = await api<{ message?: string; user?: User }>('/api/login', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (res.ok) {
      ...
      if (authMsg) authMsg.textContent = (data as any)?.message || t('messages.error');
    }
  });
  ...
  qs<HTMLButtonElement>('#reg-btn')?.addEventListener('click', async () => {
    ...
    const { res, data } = await api<{ message?: string; user?: User }>('/api/register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
